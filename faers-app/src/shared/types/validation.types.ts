...
export const SYSTEM_VALIDATION_RULES: Omit<ValidationRule, 'id' | 'createdAt' | 'updatedAt'>[] = [
  ...
  },
  {
    ...
    description: 'All reactions must be coded with MedDRA PT for submission',
    ruleType: 'required',
    severity: 'error',
    conditionExpression: "(workflow_status === 'Ready for Export' || workflow_status === 'Approved') && reaction_count > 0",
    validationExpression: 'all_reactions_coded',
    errorMessage: 'All reactions must have MedDRA coding (PT) for submission',
  ...
  },
  {
    ...
    description: 'If dose value is provided, unit should also be specified for all drugs',
    ruleType: 'cross_field',
    severity: 'warning',
    conditionExpression: 'drug_count > 0',
    validationExpression: 'all_drugs_have_dose_unit',
    errorMessage: 'Dose unit should be specified when dose value is provided (check all drugs)',
