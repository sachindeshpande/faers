...
**Implementation: ✅ Complete and empirically verified** — Feature shipped and confirmed working. IND XML generation verified: 7/7 IND test cases accepted (CA+AE) by the ZZFDATST_PREMKT test gateway as of 2026-05-09.

Key clarifications from live testing:

- **Correct test gateway name:** `ZZFDATST_PREMKT` (not `ZZFDA_PREMKT` for test environment). Production gateway is `ZZFDA_PREMKT`.
- **C.5.6.r cross-reference warning:** The `CA+AE` (Accepted with Errors) ACK for CDER_IND submissions includes a channel-inherent warning on the C.5.6.r cross-reference element. This warning cannot be suppressed — it is generated by the CDER_IND routing layer for all submissions. `CA+AE` is the expected and correct ACK for IND submissions; it does not indicate a submission error.
- **Race code policy:** All 5 FDA race codes are valid. Per `fda_e2b_r3_core_regional_data_elements_business_rules_v1.7.xlsx` row 169 the correct codes are: `C16352` = African American, `C41259` = American Indian or Alaska Native, `C41260` = Asian, `C41219` = Native Hawaiian or Other Pacific Islander, `C41261` = White. (Code `C17998` is rejected — do not use it.)
- **Empirical policy updated:** `IND_POLICY` entries in `faersEmpiricalPolicy.ts` promoted from `untested` to `proven` for all 7 accepted test cases.
