...
| **`IND_POLICY` empirical table** | Complete | `faersEmpiricalPolicy.ts` — `batchReceiver` / `msgReceiver` / `crossReportedInd` / `requiredIntervention` / `indNumber` are `proven_safe` from IND-T01 v4 CA+AE on 2026-04-27 (GAP-IND-001/002 closed) and T06 v32 CA+AE on 2026-04-28 (GAP-IND-007 closed); `indNumber.entries[]` records `123456` proven_safe + `999999` proven_rejected. The remaining IND fields stay `untested` until they ride a successful submission of their own |
| **`IND_POLICY` enforced at generate + validate (GAP-APP-001/003)** | Complete | `xmlGeneratorService.buildResearchStudy` throws on `proven_rejected` indNumber and warns on untested values; `fivePassValidatorService.indPolicySweep` walks the OID and emits Pass 3 errors for any `proven_rejected` extension |
| **IND golden + passes 1/4/5 (GAP-APP-004)** | Complete | `resolveGoldenIndPath()` locates `IND-T01-susar-baseline.xml`; when found, IND/babe cases run all 5 passes against it instead of the historical skip. Falls back to skip when the IND golden is missing |
