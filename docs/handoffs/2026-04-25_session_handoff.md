...
Sitting at `test/test_submission/from_app/ind/`, all generated post-fix and verified to carry the corrected `ZZFDA_PREMKT` receiver and `extension="2"` (or `"3"` for follow-up):
- `IND-T01-susar-baseline.xml` — basic SUSAR, 15-day, full study block
...
- `IND_POLICY`: every entry is `untested`. No live ZZFDA_PREMKT submission has happened yet. Until one does, the IND validator skips Passes 1/4/5 and the headless CLI skips the postmarket Python lint for IND/BA-BE cases — they'd produce noise without an IND-specific reference.
...
- `BATCH_RECEIVERS.{Test,Production}.Premarket = 'ZZFDA_PREMKT'` routing
...
- **`ZZFDATST_PREMKT` does not exist.** It was a misconception in this codebase that I introduced and the gap analysis caught. Test and production for Premarket both use `ZZFDA_PREMKT`. The test/prod split happens at the network endpoint URL (`api-test.fda.gov` vs `api.fda.gov`), not in the receiver ID.
