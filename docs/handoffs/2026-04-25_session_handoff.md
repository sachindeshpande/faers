...
Sitting at `test/test_submission/from_app/ind/`. **The packages generated by this session were rejected by the FDA gateway on 2026-04-27 (GAP-IND-001) — they had `ZZFDA_PREMKT` for Test, but FDA requires `ZZFDATST_PREMKT`.** Re-generate after the GAP-IND-001 fix lands. The version-id fix (`extension="2"`) was correct and stays:
